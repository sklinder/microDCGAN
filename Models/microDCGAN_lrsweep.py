...
lr_list = [0.00005, 0.0001 , 0.00015, 0.0002]

for lr in lr_list:
  
  print(f'\n\nNew job with learn rate {lr}')
  ...
  num_epochs = 25                                       # Number of training epochs (=complete passes through the training dataset)
  ...
  name_output = f'{architecture}_{size_input}x{size_input}_bs{batch_size}_lr{"{:f}".format(lr).rstrip("0").split(".")[1]}_ks{ks}_e{num_epochs}_{note}'
  ...
  device_FID = torch.device('cuda:0')
  fid = FrechetInceptionDistance(feature=2048, reset_real_features=False).to(device=device_FID)

  # Update the pretrained model with real validation data
  for batch in train_loader:
    batch = (batch[0].repeat(1,3,1,1))*255
    fid.update(batch.to(device_FID).type(torch.uint8).repeat(1,3,1,1), real=True)     # batch[0] contains the images whereas batch[1] contains the labels                     
  ...
  def FID(fake):
    ...
    '''
    fid.reset()  
    fake = (fake.repeat(1,3,1,1))*255                                        # Resets all fake features (reset_real_features=False) 
    fid.update(fake.to(device_FID), real=False)
  ...
  # Initialize plots
  if live_progress:
      # Live plot (continually updated) ## Disabled (works only for connected display)
  ...
  time_start = datetime.now()            # maybe use https://stackoverflow.com/questions/60928718/python-how-to-replace-tqdm-progress-bar-by-next-one-in-nested-loop for nested tqdm
  ...
  try:
    ...
    plt.savefig(path_output+'/'+name_output+'_loss_real_fake.png', dpi=250)
    plt.savefig(path_output+'/'+name_output+'_loss_real_fake.eps', format='eps')
  ...
  try:
    ...
    #plt.plot()
    plt.savefig(path_output+'/'+name_output+'_losses.png', dpi=250)
    plt.savefig(path_output+'/'+name_output+'_losses.eps', format='eps')
  ...
  try:
    ...
    plt.tight_layout()
    plt.savefig(path_output+'/'+name_output+'_comparison.png', dpi=250)
    plt.savefig(path_output+'/'+name_output+'_comparison.eps', format='eps')
    #plt.show()
  ...
  try:
    ...
    plt.savefig(path_output+'/'+name_output+'_nearest_Neighbour.png', dpi=250)
    ...
    for i in range (4):
        ...
        plt.savefig(path_output+'/'+name_output+f'_nearest_neighbour_{i}.png', dpi=250)
        plt.savefig(path_output+'/'+name_output+f'_nearest_neighbour_{i}.eps', format='eps')
  
  except Exception as e:
    print(e)
  
  # ##### SSIM (disabled due to high computation time)
  ...
  for iteration, frame in enumerate(frames, 1):
      ...
      try:
        cv2.imwrite(f'E_{lr}_{str(iteration).zfill(len(str(len(img_list))))}.jpg', frame)
      except Exception as e:
        print(e)
