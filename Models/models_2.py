...
class DCGAN_Generator(nn.Module):
    ...
    def __init__ (self, img_channels, num_features, stride=2, kernel_size=4):
        ...
        self.model = nn.Sequential (
            self._block (100, num_features * 16, 4, stride, 0),                               # 4x4, maybe use bias=False (like in official example)
            self._block (num_features * 16, num_features * 8, kernel_size, stride, 1),                      # 8x8
            self._block (num_features * 8, num_features * 4, kernel_size, stride, 1),                       # 16x16
            self._block (num_features * 4, num_features * 2, kernel_size, stride, 1),                       # 32x32
            self._block (num_features * 2, num_features * 1, kernel_size, stride, 1),                       # 64x64
            nn.ConvTranspose2d (num_features * 1, img_channels, kernel_size = kernel_size, stride = stride, padding = 1, bias = False), # 128x128
            nn.Tanh ()         # Tanh activation ensures that output is in [-1, 1]
        )
